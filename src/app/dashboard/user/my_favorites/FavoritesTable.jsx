...
const FavoritesTable = ({ favoriteRecipes }) => {
  ...
  const handleRemove = async (favId, recipeName) => {
    ...
    startTransition(async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/favorites/${favId}`, {
          method: 'DELETE',
        });
  ...
  if (favoriteRecipes.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 bg-gradient-to-b from-amber-50/20 to-white border border-orange-100/70 rounded-3xl p-8 text-center shadow-sm max-w-xl mx-auto">
        <div className="p-5 bg-amber-50 rounded-2xl text-amber-500 mb-5 relative">
          <FaUtensils className="text-4xl" />
          <FaHeart className="text-lg text-rose-400 absolute -top-1 -right-1 animate-ping" />
        </div>
        <h3 className="text-xl font-bold text-zinc-800 tracking-tight">Your Taste Trove is Empty</h3>
        ...
        <Link
          ...
          href="/recipes" 
          className="mt-6 bg-linear-to-r from-amber-500 to-orange-500 text-white font-semibold shadow-lg shadow-orange-500/20 hover:scale-105 transition-transform duration-200"
          radius="xl"
          size="lg"
        >
            ...
            <div className="relative h-44 w-full overflow-hidden bg-zinc-100">
              ...
              <div className="absolute top-3 left-3 z-10">
                <Chip 
                  variant="solid" 
                  size="sm"
                  className={`capitalize font-semibold shadow-sm text-white ${
                    recipe.category === 'dinner' ? 'bg-indigo-600' :
            ...
            <div className="p-4 flex-1 flex flex-col justify-between gap-4">
              ...
              <div className="flex items-center gap-2 border-t border-zinc-50 pt-3">
                <Link
                  ...
                  className="flex-1 bg-zinc-900 hover:bg-zinc-400
                   text-white font-bold text-xs shadow-md shadow-orange-600/10 transition-colors "
                  size="sm"
                  radius="lg"
                  startContent={<FaEye />}
                >
                ...
                <Button
                  isIconOnly
                  size="sm"
                  isLoading={deletingId === recipe._id && isPending}
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <Chip 
                      variant="flat" 
                      size="sm"
                      className={`capitalize font-semibold rounded-lg px-2 py-1 ${
                        recipe.category === 'dinner' ? 'bg-indigo-50 text-indigo-600' :
                    ...
                    <div className="flex items-center justify-center gap-3">
                      ...
                      <Tooltip content="View Details" closeDelay={50} color="warning" size="sm" radius="md">
                        <Button
                          ...
                          isIconOnly
                          size="md"
                          className="bg-orange-600 hover:bg-orange-700 text-white font-bold shadow-md shadow-orange-600/10 transition-colors"
                          radius="xl"
                        >
